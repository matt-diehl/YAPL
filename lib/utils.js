...
const utils = module.exports = {
    ...
    /**
     * Parse nested (or not) css (Sass style) and return a list of standard css selectors
    ...
    findRelevantSelectorsInNestedCss(css) {
        ...
        let concatNestedRules = (selector, parent, appendParent) => {
            while (parent.type === 'atrule') {
                parent = parent.parent || {};
            };

            let parentSelectors = parent && parent.selectors ? parent.selectors : [''];

            return parentSelectors.map((parentSelector) => {
                let generatedSelector = '';
                let prepend = '';
                let appendGrandParent = '';

                if (selector.indexOf('&') > 0) {
                    let parts = selector.split('&');
                    prepend = parts[0];
                    generatedSelector = parentSelector + parts[1] || '';
                } else if (parentSelector.indexOf('&') > 0) {
                    let parts = parentSelector.split('&');
                    prepend = parts[0];
                    appendGrandParent = parts[1] || '';
                    generatedSelector = selector.replace('& ', '').trim();
                } else {
                    generatedSelector = (
                        (parentSelector || '') + (appendParent || '') + (' ' + selector).replace(' &', '')
                    ).trim();
                }

                if (!parentSelector && parent.type !== 'atrule') return generatedSelector;

                return prepend + concatNestedRules(generatedSelector, parent.parent, appendGrandParent)
            });
        };
